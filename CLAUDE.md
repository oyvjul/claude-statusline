...
- `.claude/statusline-command.js` — Thin entry point (3-line wrapper, plain JS), referenced directly by `~/.claude/settings.json`
- `src/types.ts` — Shared interfaces (`StatusInput`, `UsageData`, `GitInfo`) and type alias (`RGB`)
- `src/main.ts` — Async main: stdin, line assembly, color palette, stdout
- `src/render.ts` — `renderCommitBar`, `renderBar`, `renderBadge`, `getThresholdColor`
- `src/ansi.ts` — `ansi()`, `ansiBg()`, `RESET`
- `src/usage.ts` — `getOAuthToken()`, `fetchUsage()`, cache constants, `debugLog`
- `src/git.ts` — `getGitInfo()`, `run()` helper
- `src/format.ts` — `formatReset()`
- `dist/` — Compiled JS output (gitignored), generated by `npm run build`
- `tsconfig.json` — TypeScript config (`strict: true`, `noUncheckedIndexedAccess`, ESM/nodenext)
- `install.js` — Configures `~/.claude/settings.json` to point to this repo (plain JS)

The install script:

- Removes old symlinks in `~/.claude/` that point into this repo
- Points `statusLine` in `~/.claude/settings.json` directly to the repo entry point
- Handles corrupt `settings.json` (backs up to `.bak`)
- Is idempotent (safe to re-run) — run via `npm run configure`

After cloning: `npm install && npm run build && npm run configure`
